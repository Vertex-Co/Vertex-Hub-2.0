...
export function formatCNPJ(value:string|undefined){const v=digitsOnly(value??"").slice(0,14);return v.replace(/(\d{2})(\d)/,"$1.$2").replace(/(\d{3})(\d)/,"$1.$2").replace(/(\d{3})(\d)/,"$1/$2").replace(/(\d{4})(\d{1,2})$/,"$1-$2")}
