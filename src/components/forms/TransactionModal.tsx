...
import { toLocalDateInputValue } from "../../utils/dateRanges";
...
export function TransactionModal({onClose,transaction}:{onClose:()=>void,transaction?:Transaction}) {
 const {saveTransaction}=useFinance(); const [saveError,setSaveError]=useState(""); const {register,handleSubmit,formState:{errors,isSubmitting},reset}=useForm<FormData>({resolver:zodResolver(schema),defaultValues:{type:"expense",status:"paid",date:toLocalDateInputValue(),paymentMethod:"Pix"}});
 useEffect(()=>{if(transaction)reset(transaction);},[transaction,reset]);
 const submit=async(data:FormData)=>{setSaveError("");const result=await saveTransaction(data,transaction?.id);if(result.ok)onClose();else setSaveError(result.error);};
 return <Modal title={transaction?"Editar transação":"Nova transação"} onClose={onClose}><form onSubmit={handleSubmit(submit)} className="grid gap-4 sm:grid-cols-2">
  ...
  <label>Data da transação<Input type="date" {...register("date")}/><small className="text-zinc-500">Você pode escolher uma data anterior para lançar uma transação retroativa.</small><small>{errors.date?.message}</small></label>
  ...
  {saveError&&<p role="alert" className="rounded-xl bg-red-500/10 p-3 text-sm text-red-600 dark:text-red-400 sm:col-span-2">{saveError}</p>}
  <div className="flex justify-end gap-2 sm:col-span-2"><Button type="button" variant="secondary" onClick={onClose} disabled={isSubmitting}>Cancelar</Button><Button type="submit" disabled={isSubmitting}>{isSubmitting?"Salvando...":"Salvar transação"}</Button></div>
 </form></Modal>;
}
