...
function AppGate(){const{user,loading}=useAuth();const[publicPage,setPublicPage]=useState<"landing"|"login"|"register">("landing");if(!isSupabaseConfigured)return <ConfigurationRequired/>;if(loading)return <Loading/>;if(user)return <CompanyProvider><CompanyGate/></CompanyProvider>;return publicPage==="landing"?<Landing onLogin={()=>setPublicPage("login")} onRegister={()=>setPublicPage("register")}/>:<AuthPage initialMode={publicPage}/>}
