...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const effectiveLiveMode = providerLiveMode ?? mode === "production";
    const environmentMismatch = providerLiveMode !== undefined && providerLiveMode !== (mode === "production");
    ...
    return reply({ success: true, payment: { id, order_id: order.id, external_reference: externalReference, amount, application_id: applicationId, processing_mode: order.processing_mode, status: normalizedStatus, status_detail: providerStatusDetail, payment_method: paymentMethodId, payment_method_type: paymentMethodType, environment: mode, transaction_id: transaction.id, safe_provider_data: safeData, created_at: order.date_created ?? new Date().toISOString() } });
  } catch (error) {
