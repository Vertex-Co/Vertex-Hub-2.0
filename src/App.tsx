...
function CompanyGate(){const{profile,loading,companies,reload}=useCompany();if(loading)return <Loading/>;if(!profile||!profile.isAuthorized||!profile.onboardingCompleted)return <Onboarding onCompleted={reload}/>;if(profile.globalRole!=="super_admin"&&!companies.length)return <Onboarding onCompleted={reload}/>;return <MfaGate superAdmin={profile.globalRole==="super_admin"}><AccountRequirements><FinanceProvider><FinanceApp/></FinanceProvider></AccountRequirements></MfaGate>}
function AppGate(){const{user,loading,recoveryMode}=useAuth();if(!isSupabaseConfigured)return <ConfigurationRequired/>;if(loading)return <Loading/>;if(recoveryMode)return <PasswordRecoveryPage/>;if(!user)return <AuthPage/>;return <CompanyProvider><CompanyGate/></CompanyProvider>}
