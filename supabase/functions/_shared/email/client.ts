import type { PublishedTemplateVariables, SendEmailInput, SendEmailResult } from "./types.ts";
...
export async function sendPublishedTemplate(input:{to:string;eventKey:string;templateId:string;variables:PublishedTemplateVariables;userId:string}):Promise<SendEmailResult>{
 const apiKey=(Deno.env.get("RESEND_API_KEY")??"").trim(),fromEmail=(Deno.env.get("RESEND_FROM_EMAIL")??"").trim(),fromName=(Deno.env.get("RESEND_FROM_NAME")??"Vertex Hub").trim();
 if(!EMAIL_PATTERN.test(input.to))return {ok:false,code:"INVALID_RECIPIENT",retryable:false};
 if(!apiKey||!EMAIL_PATTERN.test(fromEmail)||!input.templateId)return {ok:false,code:"RESEND_NOT_CONFIGURED",retryable:false};
 try{const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json","User-Agent":"Vertex-Hub/1.0","Idempotency-Key":input.eventKey.slice(0,256)},body:JSON.stringify({from:`${fromName} <${fromEmail}>`,to:[input.to],template:{id:input.templateId,variables:input.variables}})});const data=await response.json().catch(()=>({}));if(!response.ok||!data?.id)throw new Error(`HTTP_${response.status}`);return {ok:true,id:String(data.id)}}catch(error){console.error("email_test_failed",{userId:input.userId,provider:"resend",reason:error instanceof Error?error.message.slice(0,80):"unknown",timestamp:new Date().toISOString()});return {ok:false,code:"RESEND_SEND_FAILED",retryable:true}}
}
