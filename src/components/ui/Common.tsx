...
export function Modal({title,onClose,children}:{title:string,onClose:()=>void,children:React.ReactNode}) {
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={e=>e.target===e.currentTarget&&onClose()}>
    <div className="max-h-[94vh] w-full overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl dark:bg-zinc-900 sm:max-w-xl sm:rounded-3xl sm:p-6">
      <div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-bold">{title}</h2><button aria-label="Fechar" onClick={onClose} className="rounded-lg p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800"><X size={20}/></button></div>{children}
    </div></div>;
}
