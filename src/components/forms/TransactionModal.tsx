...
export function TransactionModal({onClose,transaction}:{onClose:()=>void,transaction?:Transaction}) {
 const {saveTransaction}=useFinance(); const [saveError,setSaveError]=useState(""); const {register,handleSubmit,formState:{errors,isSubmitting},reset}=useForm<FormData>({resolver:zodResolver(schema),defaultValues:{type:"expense",status:"paid",date:new Date().toISOString().slice(0,10),paymentMethod:"Pix"}});
