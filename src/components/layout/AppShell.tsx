...
export function AppShell({page,setPage,onNew,children}:{page:Page;setPage:(p:Page)=>void;onNew:()=>void;children:React.ReactNode}){
 ...
 useEffect(()=>{
  let alive=true;
  const load=async()=>{const{data,error}=await supabase.from("platform_notifications").select("id,title,message,type,target_type,target_value").eq("active",true).order("created_at",{ascending:false}).limit(30);if(alive&&!error)setPlatform((data??[]) as PlatformRow[])};
  void load();
  const channel=supabase.channel("vertex-platform-notifications").on("postgres_changes",{event:"*",schema:"public",table:"platform_notifications"},()=>void load()).subscribe();
  return()=>{alive=false;void supabase.removeChannel(channel)};
 },[]);
 const notifications=useMemo<FinancialNotification[]>(()=>{
  const companyId=activeCompany?.id;
  const financialItems=financial.map(item=>({...item,id:`financial:${companyId??"global"}:${item.id}`}));
  const platformItems:FinancialNotification[]=platform.filter(item=>item.target_type==="all"||(item.target_type==="company"&&item.target_value===companyId)).map(item=>({id:`platform:${item.id}`,title:item.title,description:item.message,page:"dashboard",level:item.type==="important"||item.type==="maintenance"?"danger":"info"}));
  return[...platformItems,...financialItems];
 },[activeCompany?.id,financial,platform]);
 const go=(target:Page)=>{setPage(target);setMobile(false)};
 const name=profile?.fullName||String(user?.email?.split("@")[0]??"Usuário");
 const navItem=([id,label,Icon]:[Page,string,React.ElementType])=><button key={id} onClick={()=>go(id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${page===id?"bg-blue-600 text-white":"text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-900"}`}><Icon size={18}/>{label}</button>;
 return <div className="min-h-screen bg-zinc-100 text-zinc-900 transition-colors dark:bg-zinc-950 dark:text-zinc-50">
  <aside className={`fixed inset-y-0 left-0 z-40 w-72 overflow-y-auto border-r bg-white p-5 transition-transform dark:border-zinc-800 dark:bg-zinc-950 lg:translate-x-0 ${mobile?"translate-x-0":"-translate-x-full"}`}>
   <div className="mb-7 flex items-center justify-between"><button onClick={()=>go("dashboard")} className="flex items-center gap-3 text-xl font-black"><OfficialLogo className="size-11"/><span className="text-left">Vertex Hub<span className="block text-[10px] font-medium text-zinc-500">{isSuperAdmin?"BACKOFFICE ADMIN":"Central de Gestão Digital"}</span></span></button><button className="lg:hidden" onClick={()=>setMobile(false)}><X/></button></div>
